...
class EdgeConnect():
    ...
    def train(self):
        train_loader = DataLoader(dataset=self.train_dataset, batch_size=self.config.BATCH_SIZE, num_workers=4,
                                  drop_last=True, shuffle=True)
        epoch_start = self.edge_model.epoch
        keep_training = True
        model = self.config.MODEL
        epochs = int(float(self.config.EPOCHS))
        ...
        for epoch in range(epoch_start, epochs):
            ...
            for items in train_loader:
                ...
                if model == 1:
                    ...
                    iteration += self.config.BATCH_SIZE
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            name = self.test_dataset.load_name(index).replace('.mat', '.png')
            images, edges, masks = self.cuda(*items)
            index += 1
            ...
            # inpaint with edge model / joint model
                ...
                edges_out = self.edge_model(images, edges, masks).detach()
            ...
            if self.debug:
                edges = self.postprocess(1 - edges_out)[0]
    ...
    def sample(self, epoch, iteration):
        ...
        name = os.path.join(path, str(epoch).zfill(3) + '_' + str(iteration).zfill(5) + ".png")
        create_dir(path)
        print('\nsaving sample ' + name)
        images.save(name)
