...
# Downloading: "https://download.pytorch.org/models/inception_v3_google-1a9a5a14.pth" to /home/czey/.cache/torch/checkpoints/inception_v3_google-1a9a5a14.pth
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    ...
    if os.path.exists(npz_file):
        ...
        files = list(path.glob('*.npy')) + list(path.glob('*.mat'))

        # imgs = np.array([imread(str(fn)).astype(np.float32) for fn in files])
        imgs = np.array([np.repeat(np.load(str(files[0]))[np.newaxis,:,:],3, axis=0).astype(np.float32) for fn in files])
