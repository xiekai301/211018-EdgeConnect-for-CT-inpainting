...
random.seed(2021)  # sucess, also can replace '.npy' by '_edge.png' in CTArm_train.list


imgtype = 'npy'
if imgtype == 'npy':
    train_filename = './datasets/CTArm_train.flist'
    val_filename = './datasets/CTArm_val.flist'
else:
    train_filename = './datasets/CTArm_edge_train.flist'
    val_filename = './datasets/CTArm_edge_val.flist'

parser = argparse.ArgumentParser()
parser.add_argument('--folder_path', default='/home/czey/generative_inpainting/training_data/CTArmNpy', type=str,
                    help='The folder path')
parser.add_argument('--train_filename', default=train_filename, type=str,
                    help='The train filename.')
parser.add_argument('--validation_filename', default=val_filename, type=str,
...
if __name__ == "__main__":
    ...
    for validation_dir in sorted(validation_dirs):
        # append each file into the list file names
        validation_folder = sorted(os.listdir(args.folder_path + "/validation" + "/" + validation_dir))
        for validation_item in validation_folder:
            # modify to full path -> directory
            if imgtype in validation_item:
